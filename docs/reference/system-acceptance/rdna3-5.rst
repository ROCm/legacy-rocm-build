...
targets) handle memory access through GPU Virtual Memory (GPUVM), which
...
.. note::
   ...
   (for example, 0.5 GB) and increase the shared (TTM/GTT) limit instead.
...
.. note::
  ...
  The amd-ttm command converts the pages to GB to help the users.
...
:ref:`Operating system support <release-supported-os>`.
