...
Bare Metal follows the routine {doc}`ROCm installation procedure <../../install/rocm>`.
...
3. In the **Virtual Machine Manager** GUI, select the **VM** and click **Open**.
   ...
   ![Virtual Machine Manager](./images/tuning014.png "Virtual Machine Manager")

4. In the VM GUI, go to **Show Virtual Hardware Details > Add Hardware** to
  configure hardware.

   ![Show virtual hardware details](./images/tuning015.png "Virtual Machine Manager: show virtual hardware details")

5. Go to **Add Hardware > PCI Host Device > VF** and click **Finish**.

    ![VF Selection](./images/tuning016.png "VF Selection")

Then start the VM.

Lastly, install ROCm on the virtual machine (VM). For detailed instructions,
refer to the {doc}`Linux install guide </install/rocm>`.
